...
public class AppException extends Exception {
	...
	private byte type;
	private int code;
	private String exceptionMessage = "发生异常";
	...
	private AppException(byte type, int code, Exception excp,String message) {
		super(excp);
		this.type = type;
		this.code = code;
	...
	public void saveErrorLog(Exception excp) {
		...
		try {
			...
			if(storageState.equals(Environment.MEDIA_MOUNTED)){
				savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/netmoon/Log/";
	...
	public static AppException createNetworkException(Exception e ,String msg) {
		if(msg!=null){
			return new AppException(TYPE_NETWORK, 0, e,msg);
		}else if(e instanceof UnknownHostException ){
			return new AppException(TYPE_NETWORK, 0, e,"找不到服务器");
		}else if(e instanceof ConnectException){
			return new AppException(TYPE_NETWORK, 0, e,"连接失败");
		}else if(e instanceof UnsupportedEncodingException){
			return new AppException(TYPE_NETWORK, 0, e,"服务器返回异常");
			//return new AppException(TYPE_NETWORK, 0, e,"编码异常");
		}else if(e instanceof MalformedURLException){
			return new AppException(TYPE_NETWORK, 0, e,"请求的URL地址有误");	
		}else if(e instanceof ClientProtocolException){
			return new AppException(TYPE_NETWORK, 0, e,"发生请求错误");
		}
